      ...
      <div className="min-h-screen bg-[#fafbfc] text-[#4a5568] flex flex-col justify-between font-sans">
        ...
        <main className="max-w-md mx-auto px-6 py-24 text-center">
          <div className="bg-white p-8 rounded-2xl border border-gray-100 shadow-sm">
            ...
            <Link href="/magazine" className="px-5 py-2.5 bg-[#88abac] hover:bg-[#789b9c] text-white text-xs rounded-xl font-medium transition shadow-sm">
              返回杂志列表
            </Link>
          </div>
        </main>

        <footer className="w-full bg-[#88abac] text-white py-6 px-8">
...
export async function getStaticPaths() {
  try {
    ...
    const { data } = await supabase.from('magazines').select('id');
